...
from utils import get_colours_for_year, get_runs_by_year, seconds_to_time
...
def draw_axes(svg):
    ...
    while pace <= MAX_PACE:
        y = scale_pace(pace)
        svg.add('line', {'x1': x1 - 6, 'y1': y, 'x2': x2, 'y2': y, 'class': 'axis'})
        svg.add('text', {'x': x1 - 12, 'y': y, 'class': 'y-axis-label'}, seconds_to_time(pace * 60))
        pace += 0.25
...
def draw_year_line(svg, data, year):
    ...
    text_x = points[-1][0] + 5 + points[-1][2] * SCALE_DIST
    text_y = points[-1][1]
    svg.add('text', {'x': text_x, 'y': text_y, 'class': 'series-label'}, year)
